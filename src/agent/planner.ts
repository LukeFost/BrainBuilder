...
6.  **Efficiency:** Choose the most direct sequence. Use \`moveToPosition\` only when necessary to reach resources, crafting stations, or targets. Avoid unnecessary movement. Consider tool efficiency (see Tool Check & Crafting).
7.  **Error Handling:** If the 'Last Action Result' indicates a failure, the new plan MUST address the cause. Examples: If \`craftItem\` fails with 'Need crafting table nearby', the next plan MUST include \`placeBlock crafting_table\` at a suitable empty location before retrying the craft. If \`collectBlock\` fails with 'Need a suitable tool', the next plan MUST include crafting the required tool. If \`placeBlock\` fails, try a different nearby empty location. Avoid repeating the exact failed action immediately.
...
9.  **Stuck Detection:** If the same action fails multiple times (see Last Action Result) or progress isn't being made towards the goal despite several steps, use \`askForHelp\` or try a significantly different approach (e.g., explore elsewhere if resources aren't found).
10. **Safety:** If health is low (e.g., < 10), prioritize safety: avoid combat, find/eat food if available, or \`askForHelp\`. If hunger is low (e.g., < 6), plan to find and eat food soon.
11. **Output Format:** Output ONLY the list of planned actions, one action per line. Do NOT include explanations, numbering, comments, or any introductory/concluding text. Ensure each line is a valid action call (e.g., \`collectBlock oak_log 5\`, \`craftItem crafting_table 1\`).
